...
st.write(f"현재 체중: {temperature}kg")
...
st.write("키와 몸무게를 입력하면 BMI를 계산하고, 현재 상태를 알려드립니다.") 
         
st.write("BMI = 체중(kg) / (신장(m) * 신장(m)")
...
if bmi < 18.5:
    status = "저체중"
    detail = "체중을 약간 늘리는 것을 고려해 보세요.🐣"
elif 18.5 <= bmi < 23:
    status = "정상 체중"
    detail = "아주 좋아요! 현재 상태를 잘 유지해 보세요.🐹"
elif 23 <= bmi < 25:
    status = "과체중(주의)"
    detail = "가벼운 운동과 식단 조절을 시작해 보는 게 좋겠습니다.🐻"
elif 25 <= bmi < 30:
    status = "비만 1단계"
    detail = "꾸준한 운동과 식습관 개선이 필요합니다.🐷"
else:
    status = "비만 2단계 이상"
    detail = "전문의와 상담하며 체계적인 관리를 고려해 보세요.⚠️"

st.success(f"현재 상태: **{status}**")
st.write(detail)  # ← 이 줄을 추가해야 문장이 보여요
    ...
    st.caption("※ BMI는 참고용 지표이며, 개인의 체형·근육량·건강 상태에 따라 해석이 달라질 수 있습니다.")
